...
router.get('/events/:id/check-registration', [
  ...
  try {
    ...
    // Check registration by phone (works for both active and deleted members)
    let registration = await EventRegistration.findOne({
      where: { eventId, memberPhone: phone },
      include: [
    ...
    // Fallback: If not found directly by phone in registration table (for older records)
    if (!registration) {
      ...
      if (member) {
        registration = await EventRegistration.findOne({
          ...
          include: [{ model: Event, as: 'event' }, { model: Member, as: 'member' }]
        });
      }
    }
    ...
    if (!registration) {
      return res.status(200).json({
        success: true,
        isRegistered: false,
        message: 'No registration found for this event. You can register now.'
      });
    }

    // Use member data from association if available, otherwise use snapshot
    const effectiveMember = registration.member || {
      id: null,
      name: registration.memberName,
      phone: registration.memberPhone,
      profileImage: null
    };
    ...
    const profileMeta = ensureProfileImageUrl(
      effectiveMember.profileImageURL || effectiveMember.profileImage || null,
      baseUrl
    );
    ...
    res.status(200).json({
      ...
      member: {
        ...
        phone: effectiveMember.phone,
        email: effectiveMember.email || null,
        businessName: effectiveMember.businessName || null,
        associationId: effectiveMember.associationId || null,
        associationName: effectiveMember.associationName || null,
        profileImage: profileMeta.stored || effectiveMember.profileImage || null,
        profileImageURL,
        city: effectiveMember.city || null
      },
...
router.post('/events/:id/registrations/:registrationId/send-whatsapp',
  ...
  async (req, res) => {
    try {
      ...
      // Get member details either from association or snapshot
      const effectiveMember = registration.member || {
        id: null,
        name: registration.memberName,
        phone: registration.memberPhone
      ...
      const memberPhone = effectiveMember.phone;
      if (!memberPhone || memberPhone.trim() === '') {
        console.error(`[WhatsApp Send] Phone number missing for registration ${registrationId}`);
        return res.status(400).json({
          success: false,
          message: 'Member phone number is required but not found.'
        });
      }
      ...
      const memberName = effectiveMember.name || '';
