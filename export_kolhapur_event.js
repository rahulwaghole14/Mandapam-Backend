...
async function exportKolhapurEventToCSV() {
  try {
    ...
    const kolhapurEventsQuery = `
      SELECT id, title, description, city, event_date, venue, created_at
      FROM events 
    ...
    if (kolhapurEventsResult.rows.length === 0) {
      ...
      const allEventsQuery = `
        SELECT id, title, city, event_date, venue
        FROM events 
        ORDER BY event_date DESC
        LIMIT 10
      `;
      ...
      allEventsResult.rows.forEach(event => {
        console.log(`  ID: ${event.id}, Title: ${event.title}, City: ${event.city}, Date: ${event.event_date}`);
      });
      return;
    }
    ...
    kolhapurEventsResult.rows.forEach(event => {
      console.log(`  Event ID: ${event.id}, Title: ${event.title}, City: ${event.city}, Date: ${event.event_date}, Venue: ${event.venue}`);
    });
...
${kolhapurEventsResult.rows.map(e => `- ${e.title} (ID: ${e.id}, Date: ${e.event_date})`).join('\n')}
